...
describe('PrnParser', function() {
  ...
  it('has 4 invoices', function() {
    var parser = PrnParser(prnFile);

    expect(parser.invoices.length).to.equal(4);
  });

  describe('lineItems', function() {
    it('1st invoice has 31 items', function() {
      var parser  = PrnParser(prnFile);
      var invoice = parser.invoices[0];

      expect(invoice.invoice.number).to.equal('MI-A080804');
      expect(invoice.lineItems.length).to.equal(31);
    });

    it('2nd invoice has 7 items', function() {
      var parser  = PrnParser(prnFile);
    ...
    it('3nd invoice has 7 items', function() {
      var parser  = PrnParser(prnFile);
    ...
    it('4th invoice has 3 items', function() {
      var parser  = PrnParser(prnFile);
