...
ExcelUpdater.prototype = {
  ...
  updateHeader: function(invoice, startingRow) {
    this.setCell('B' + (1 + startingRow), 'វិក្កយប័ត្រអាករ', this.khmerFontOptions);
    this.setCell('B' + (2 + startingRow), 'VAT INVOICE');
    this.setCell('B' + (3 + startingRow), 'លេខអត្តសញ្ញាណកម្ម អតប', this.khmerFontOptions);
    this.setCell('B' + (4 + startingRow), 'VAT TIN: ' + invoice.tin);
    this.setCell('B' + (6 + startingRow), 'ឈ្មោះអតិថិជន', this.khmerFontOptions);
    ...
    this.setCell('G' + (15 + startingRow), 'កាលបរិចេ្ឆទនៃកិច្ចសន្យា', this.khmerFontOptions);
    ...
    this.setCell('B' + (17 + startingRow), 'ទំនាក់ទំនង', this.khmerFontOptions);
    this.setCell('B' + (18 + startingRow), 'Contact:');
    this.setCell('C' + (18 + startingRow), invoice.contact);

    this.setCell('G' + (17 + startingRow), 'រយ:ពេលទូទាត់', this.khmerFontOptions);
    this.setCell('I' + (17 + startingRow), '៧ថ្ងៃបន្ទាប់ពីទទួលបានវិក្កយប័ត្រ', { alignment: { wrapText: true } });
    this.setCell('G' + (18 + startingRow), 'Terms:   7 days after the');
    this.setCell('G' + (19 + startingRow), 'date of receiving invoice');

    this.setCell('B' + (22 + startingRow), 'លេខរៀង', this.khmerFontOptions);
    this.setCell('C' + (22 + startingRow), 'កូតទំនិញេ/សេវា', this.khmerFontOptions);
    this.setCell('D' + (22 + startingRow), 'បរិយាយមុខទំនិញ/សេវា', this.khmerFontOptions);
    this.setCell('G' + (22 + startingRow), 'បរិមាណ', this.khmerFontOptions);
    this.setCell('I' + (22 + startingRow), 'ចំនួនទឹកប្រាក់', this.khmerFontOptions);
  ...
  updateFooter: function(invoice, startingRow) {
    if (invoice.subTotal) {
      ...
      this.setCell('E' + (rowNumber + 1), 'សរុបទំព័រទី១​', this.khmerFontOptions);
      ...
      this.setCell('F' + (rowNumber + 1), 'សរុប', this.rightAlignmentKhmerFont);
      ...
      this.setCell('F' + (rowNumber + 2), 'អាករលើតម្លែបន្ថែម ១០%', this.rightAlignmentKhmerFont);
      ...
      this.setCell('F' + (rowNumber + 5), 'សរុបរួម', this.rightAlignmentKhmerFont);
      ...
      // terms
      this.setCell('B' + (rowNumber + 8), 'ការទូទាត់អាចធ្វើឡើងតាមមូលឡបទានប័ត្រ', this.khmerFontOptions);
      this.setCell('B' + (rowNumber + 9), 'ឬ តាមរយ:ការទូទាត់ទៅធនាគារ', this.khmerFontOptions);
      ...
      this.setCell('I' + (rowNumber + 8), 'ហត្ថលេខា', this.khmerFontOptions);
  ...
  setCell: function(cell, value, options) {
    ...
    if (options && options.font) {
      this.worksheet.getCell(cell).font = options.font;
    }
  }
};
